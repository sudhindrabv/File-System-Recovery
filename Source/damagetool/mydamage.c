...
void remove1(const char *src, const struct stat *srcst)
{
    ...
    if (customunlink(src, dmg_type) < 0) {
	report(src);
    } else {
...
int 
mydamage( int num, char *pname, char *pname2)
{
    int i;
    char *flags;
    struct stat st;
    ...
    prog_name= "damageFileSystemTool";
    /* Required action. */
    switch (num) {
        case 0: /* Copy */
	identity= CP;
...
int main() {
    ...
    do 
    {
        printf("\n0. Delete file without damaging folder");
        printf("\n1. Damage the inode bit map by removing the file");
        printf("\n2. Corrupt the Directory file completely");
