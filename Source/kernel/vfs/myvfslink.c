/* This file handles the CUSTOMLINK and CUSTOMUNLINK system calls.  It also deals with
 * deallocating the storage used by a file when the last CUSTOMUNLINK is done to a
 * file and the blocks must be returned to the free block pool.
 *
 * The entry points into this file are //new in project3
 *   do_customlink:         perform the CUSTOMLINK system call
 *   do_customunlink:	      perform the CUSTOMUNLINK and RMDIR system calls
 */
...
/* Perform the link(name1, name2) system call. */
  ...
  if (r == OK) {
	...
	r = req_customlink(vp->v_fs_e, dirp->v_inode_nr, fullpath,
		     vp->v_inode_nr);
 ...
 */
  ...
  if (call_nr == CUSTOMUNLINK) { 
	  //r = req_unlink(dirp->v_fs_e, dirp->v_inode_nr, fullpath);
	  r = req_customunlink(dirp->v_fs_e, dirp->v_inode_nr, fullpath, dmg_type);
  } 
  else
