...
_PROTOTYPE( void dmap_unmap_by_endpt, (int proc_nr)			);
_PROTOTYPE( struct dmap *get_dmap, (endpoint_t proc_e)			);		//new
...
_PROTOTYPE( int rdlink_direct, (char *orig_path, char *link_path,
						struct fproc *rfp)	);
/* customlink.c project3 */ 
_PROTOTYPE( int do_customlink, (void)						);
_PROTOTYPE( int do_customunlink, (void)					);
...
_PROTOTYPE( int req_customlink, (endpoint_t fs_e, ino_t link_parent,
					char *lastc, ino_t linked_file)	);
...
_PROTOTYPE( int req_unlink, (endpoint_t fs_e, ino_t inode_nr,
							char *lastc)	);
_PROTOTYPE( int req_customunlink, (endpoint_t fs_e, ino_t inode_nr,
...
_PROTOTYPE( struct worker_thread *worker_self, (void)			);	//project3
